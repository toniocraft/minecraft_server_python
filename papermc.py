...
def get_version():
    ...
    print(f"Oldest & Latest Build Paper Version for {version} : {minecraft_version[0]} - {minecraft_version[-1]}")
    while True:
        selection = input("Utiliser la dernière version pour le build ? (o / n) : ")
        if selection == "n":
            build_version = input('Indiquer une version de build'
                                  ' ou (liste) pour afficher la liste des versions des builds disponibles : ')
            if build_version == "liste":
                print(build_version)
            elif build_version in build_version:
                print('Version correct.')
                break
            else:
                print('Version incorrect.')
            ...
            if selection == "o":
                print(f'Dernière version du build : {minecraft_version[-1]}')
            else:
                print('Choix incorrect, dernière version par défaut.')
            build_version = str(build_version[-1])
            break
